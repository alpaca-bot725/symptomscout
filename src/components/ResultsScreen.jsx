...
const URGENCY_STYLES = {
  self_care: { badge: 'bg-emerald-100 text-emerald-800', banner: 'border-emerald-300 bg-emerald-50 text-emerald-900', icon: '🏠' },
  see_doctor: { badge: 'bg-amber-100 text-amber-800', banner: 'border-amber-300 bg-amber-50 text-amber-900', icon: '🩺' },
  urgent_care: { badge: 'bg-orange-100 text-orange-800', banner: 'border-orange-300 bg-orange-50 text-orange-900', icon: '⏰' },
  emergency: { badge: 'bg-red-100 text-red-800', banner: 'border-red-300 bg-red-50 text-red-900', icon: '🚨' },
}
  ...
  if (results.length === 0) {
    return (
      <div className="space-y-4">
        <div className="rounded-2xl border-2 border-slate-300 bg-white p-5 text-center text-slate-800">
  ...
  return (
    <div className="space-y-4">
      ...
      {results.map((r, index) => {
        ...
        return (
          <details key={r.condition.id} open={index === 0} className="group rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
            <summary className="flex min-h-14 cursor-pointer list-none items-center gap-3 p-4 [&::-webkit-details-marker]:hidden">
              <div className="min-w-0 flex-1">
                <h3 className="font-bold text-slate-900">{r.condition.name}</h3>
                <div className="mt-1 flex flex-wrap items-center gap-2">
                  <span className="text-sm font-semibold text-blue-700">{r.percent}% match</span>
            ...
            <div className="space-y-3 border-t border-slate-100 p-4 text-sm">
              <p className="leading-relaxed text-slate-700">{r.condition.description}</p>

              <div>
                <h4 className="font-semibold text-slate-800">Why this matched</h4>
                <ul className="mt-1 space-y-1">
                  {r.matchedSymptoms.map((s) => (
                    <li key={s.id} className="flex items-start gap-2 text-slate-700">
                      <span aria-hidden="true" className={s.hallmark ? 'text-blue-600' : 'text-slate-400'}>
                        {s.hallmark ? '◆' : '◇'}
                      </span>
                      <span>
                        {s.label}
                        {s.hallmark && <span className="ml-1 text-xs font-medium text-blue-700">(hallmark symptom)</span>}
                      </span>
                    </li>
                  ))}
                </ul>
              ...
              {r.triggeredFlags.length > 0 && (
                <div className="rounded-xl bg-orange-50 p-3 text-orange-900 ring-1 ring-orange-200">
              ...
              {r.modifiers.length > 0 && (
                <p className="text-xs text-slate-500">Urgency adjusted for: {r.modifiers.join('; ')}.</p>
              )}

              <div>
                <h4 className="font-semibold text-slate-800">What helps</h4>
                <p className="mt-1 leading-relaxed text-slate-700">{r.condition.advice}</p>
                <p className="mt-1 text-xs text-slate-500">
              ...
              {r.watchFor.length > 0 && (
                <div className="rounded-xl bg-red-50 p-3 text-red-900 ring-1 ring-red-200">
